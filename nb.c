...
typedef U1    *U1RefVar    ;
typedef U8    *U8RefVar    ;
typedef U16   *U16RefVar   ;
typedef U24   *U24RefVar   ;
typedef U32   *U32RefVar   ;
typedef U64   *U64RefVar   ;
typedef U128  *U128RefVar  ;

typedef U1RefVar    const U1Ref    ;
typedef U8RefVar    const U8Ref    ;
typedef U16RefVar   const U16Ref   ;
typedef U24RefVar   const U24Ref   ;
typedef U32RefVar   const U32Ref   ;
typedef U64RefVar   const U64Ref   ;
typedef U128RefVar  const U128Ref  ;

typedef U1Var    *U1VarRefVar    ;
typedef U8Var    *U8VarRefVar    ;
typedef U16Var   *U16VarRefVar   ;
typedef U24Var   *U24VarRefVar   ;
typedef U32Var   *U32VarRefVar   ;
typedef U64Var   *U64VarRefVar   ;
typedef U128Var  *U128VarRefVar  ;

typedef U1VarRefVar    const U1VarRef    ;
typedef U8VarRefVar    const U8VarRef    ;
typedef U16VarRefVar   const U16VarRef   ;
typedef U24VarRefVar   const U24VarRef   ;
typedef U32VarRefVar   const U32VarRef   ;
typedef U64VarRefVar   const U64VarRef   ;
typedef U128VarRefVar  const U128VarRef  ;
...
/*    */
/*  typedef U8 *U8RefVar; typedef U8RefVar const U8Ref;  */
...
typedef struct Buf BufVar;
typedef BufVar const Buf;
typedef BufVar *BufVarRefVar;
typedef BufVarRefVar const BufVarRef;
typedef Buf *BufRefVar;
typedef BufRefVar const BufRef;
struct Buf {
  U8VarRefVar in;
  size_t at;
  size_t end;
...
static inline void buf_skip(BufVarRef buf, size_t const by) {
  assert(by < buf->end);
...
typedef union V16U8 V16U8Var; typedef V16U8Var const V16U8;
union V16U8 {
  U8 vec[16];
...
#define till(to) \
  while ((BUF.at % (to)) != (size_t)0)

typedef typeof(__LINE__) LineVar;
typedef LineVar const Line;
#define LineFmt "d"

#define not_same(type, fmt, file, line, tag, exp_a, val_a, exp_b, val_b) \
  fprintf(                                                      \
    stderr,                                                     \
    "%s: "                                                      \
    "%s:"                                                       \
...
#define must_be_same(a, b)                                      \
  _Generic((a),                                                 \
           U32Var: same_u32)                                    \
  (__FILE__, __LINE__, false, #a, (a), #b, (b))

#define should_be_same(a, b)                                    \
  _Generic((a),                                                 \
           U32Var: same_u32)                                    \
  (__FILE__, __LINE__, true, #a, (a), #b, (b))
...
/*  #define Fmt(f,x)                                                \  */
/*    _Generic((x),                                                 \  */
/*             U32Var: eval(PRI##f##32))  */

/*  #define be_same(how, a, b)                                      \  */
/*    (                                                             \  */
/*      (a) == (b) ?                                                \  */
/*      (a) :                                                       \  */
/*      (how(#a, #b, (a), (b)), (b))                                \  */
...
typedef struct Work WorkVar;
typedef WorkVar *WorkVarRefVar;
typedef WorkVarRefVar const WorkVarRef;
typedef WorkVar const Work;

typedef bool Begin(WorkVarRef work, Str name);
typedef bool Mark(WorkVarRef work, Str name);
typedef bool End(WorkVarRef work, Str name);
...
bool handle([[maybe_unused]] WorkVarRef work) {
  return false;
}
...
#define declare_field(name, count, sub)                         \
  [[maybe_unused]]                                              \
  size_t of(name, sub)[count] = {}

#define declare_struct(name, count)                             \
  declare_field(name, count, begin);                            \
  declare_field(name, count, end);

#define define(name, idx, sub)                                  \
  [[maybe_unused]]                                              \
  of(name, of(sub, idx)):                                       \
  fprintf(                                                      \
    stderr,                                                     \
...
#define end(name, idx)                                          \
  ...
  //  memset((void *)file_begin[0], 0xBA, sizeof reow);

  bool const fat_binary_enabled = false;
  ...
  if (fat_binary_enabled) {
    ...
    begin(fat_header, 0);
    define(fat_header, 0, magic);
    out_u32(must_be_same(u32(0xCAFEBABE), U32(FAT_MAGIC)));
    define(fat_header, 0, nfat_arch);
    out_u32(u32(1));
    end(fat_header, 0);

    begin(fat_arch, 0);

    define(fat_arch, 0, cputype);
    out_u32(must_be_same(u32(0x0100000C), U32(CPU_TYPE_ARM64)));

    define(fat_arch, 0, cpusubtype);
    out_u32(must_be_same(u32(0x00000000), U32(CPU_SUBTYPE_ARM64_ALL)));

    define(fat_arch, 0, offset);
    // TODO
    out_u32(should_be_same(u32(0xBABABABA), U32(mach_object_begin[0] - file_begin[0])));

    define(fat_arch, 0, size);
    // TODO: size
    out_u32(should_be_same(u32(0xBABABABA), U32(file_end[0] - file_begin[0])));

    define(fat_arch, 0, align);
    // mach/vm_page_size.h:vm_page_shift
    out_u32(must_be_same(must_be_same(u32(14), U32(vm_page_shift)), U32(PAGE_SHIFT)));

    define(fat_arch, 0, reserved);
  ...
  define(mach_header_64, 0, magic);
  out_u32(must_be_same(u32(0xFEEDFACF), MH_MAGIC_64));

  define(mach_header_64, 0, cputype);
  ...
  define(mach_header_64, 0, cpusubtype);
  out_u32(must_be_same(u32(0x00000000), U32(CPU_SUBTYPE_ARM64_ALL)));

  define(mach_header_64, 0, filetype);
  out_u32(must_be_same(u32(0x00000002), U32(MH_EXECUTE)));

  define(mach_header_64, 0, ncmds);
  out_u32(must_be_same(u32(0x00000011), u32(17)));

  define(mach_header_64, 0, sizeofcmds);
  out_u32(must_be_same(u32(0x00000420), u32(1056)));

  define(mach_header_64, 0, flags);
  out_u32(must_be_same(
    u32(0x00200085),
  ...
  define(mach_header_64, 0, reserved);
  out_u32(u32(0x00000000));
  ...
  define(segment_command_64, 0, cmd);
  out_u32(must_be_same(u32(0x00000019), U32(LC_SEGMENT_64)));

  define(segment_command_64, 0, cmdsize);
  out_u32(u32(0x00000048));

  define(segment_command_64, 0, segname);
  out_v16u8((V16U8){.str = u8"__PAGEZERO"});

  define(segment_command_64, 0, vmaddr);
  out_u64(u64(0x00000000'00000000));

  define(segment_command_64, 0, vmsize);
  out_u64(u64(0x00000001'00000000));

  define(segment_command_64, 0, fileoff);
  out_u64(u64(0x00000000'00000000));

  define(segment_command_64, 0, filesize);
  out_u64(u64(0x00000000'00000000));

  define(segment_command_64, 0, maxprot);
  out_u32(u32(0x00000000));

  define(segment_command_64, 0, initprot);
  out_u32(u32(0x00000000));

  define(segment_command_64, 0, nsects);
  out_u32(u32(0x00000000));

  define(segment_command_64, 0, flags);
  out_u32(u32(0x00000000));
  ...
  define(segment_command_64, 1, cmd);
  out_u32(must_be_same(u32(0x00000019), U32(LC_SEGMENT_64)));

  define(segment_command_64, 1, cmdsize);
  out_u32(u32(0x00000188));

  define(segment_command_64, 1, segname);
  //ol128(u128(u64(0x45545F5F'00005458), u64(0x00000000'00000000)));
  out_v16u8((V16U8){.str = u8"__TEXT"});

  define(segment_command_64, 1, vmaddr);
  out_u64(u64(0x00000001'00000000));

  define(segment_command_64, 1, vmsize);
  out_u64(u64(0x00000000'00004000));

  define(segment_command_64, 1, fileoff);
  out_u64(u64(0x00000000'00000000));

  define(segment_command_64, 1, filesize);
  out_u64(u64(0x00000000'00004000));

  define(segment_command_64, 1, maxprot);
  out_u32(u32(0x00000005));

  define(segment_command_64, 1, initprot);
  out_u32(u32(0x00000005));

  define(segment_command_64, 1, nsects);
  out_u32(u32(0x00000004));

  define(segment_command_64, 1, flags);
  out_u32(u32(0x00000000));

  begin(section_64, 0);

  define(section_64, 0, sectname);
  out_v16u8((V16U8){.str = u8"__text"});
  define(section_64, 0, segname);
  out_v16u8((V16U8){.str = u8"__TEXT"});
  define(section_64, 0, addr);
  out_u64(u64(0x00000001'00000460));
  define(section_64, 0, size);
  out_u64(u64(0x00000000'0000003c));
  define(section_64, 0, offset);
  out_u32(u32(0x00000460));
  define(section_64, 0, align);
  out_u32(u32(0x00000002));

  define(section_64, 0, reloff);
  out_u32(u32(0x00000000));
  define(section_64, 0, nreloc);
  out_u32(u32(0x00000000));

  define(section_64, 0, flags);
  out_u32(must_be_same(
    u32(0x80000400),
  ...
  define(section_64, 0, reserved1);
  out_u32(u32(0x00000000));
  define(section_64, 0, reserved2);
  out_u32(u32(0x00000000));
  define(section_64, 0, reserved3);
  out_u32(u32(0x00000000));
  ...
  define(section_64, 1, sectname);
  out_v16u8((V16U8){.str = u8"__stubs"});
  define(section_64, 1, segname);
  out_v16u8((V16U8){.str = u8"__TEXT"});
  define(section_64, 1, addr);
  out_u64(u64(0x00000001'0000049c));
  define(section_64, 1, size);
  out_u64(u64(0x00000000'0000000c));
  define(section_64, 1, offset);
  out_u32(u32(0x0000049c));
  define(section_64, 1, align);
  out_u32(u32(0x00000002));

  define(section_64, 1, reloff);
  out_u32(u32(0x00000000));
  define(section_64, 1, nreloc);
  out_u32(u32(0x00000000));

  define(section_64, 1, flags);
  out_u32(must_be_same(
    u32(0x80000408),
    ( ( must_be_same(u32(0x000000FF), U32(SECTION_TYPE))
  ...
  define(section_64, 1, reserved1);
  out_u32(u32(0x00000000));
  define(section_64, 1, reserved2);
  define(section_64, 1, stub_size);
  out_u32(u32(0x0000000c));
  define(section_64, 1, reserved3);
  out_u32(u32(0x00000000));
  ...
  define(prebound_dylib_command, 0, cmd);
  out_u32(must_be_same(u32(0x00000010), U32(LC_PREBOUND_DYLIB)));
  define(prebound_dylib_command, 0, cmdsize);
  out_u32(u32(0x00008000));
  define(prebound_dylib_command, 0, name);
  out_u32(u32(0x00000060));
  define(prebound_dylib_command, 0, nmodules);
  out_u32(u32(0x80000033));
  define(prebound_dylib_command, 0, linked_modules);
  out_u32(u32(0x00000010));
